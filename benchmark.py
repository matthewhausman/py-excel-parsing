...
def iter_excel_calamine_pandas_monkeypatch(file: IO[bytes]) -> Iterator[dict[str, object]]:
    yield from pandas.read_excel(file, converters={'date': lambda ts: ts.date()}, engine="calamine").to_dict('records')
...
for fn in (
    ...
    iter_excel_calamine_pandas_monkeypatch,
    iter_excel_calamine
):
    print(f'\n{fn.__qualname__}')
